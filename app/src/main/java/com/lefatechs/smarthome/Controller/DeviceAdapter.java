...
public class DeviceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public DeviceAdapter(Context context, ArrayList<Device> itemList) {
        mContext = context;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (device.getDeviceViewType() == 1) {
            ...
            if (!device.isPower()) {
                ...
                ((ViewHolderSeekBar) holder).mCardView.setElevation(3);
            ...
            if (device.isPower()==false) {
                ...
                ((ViewHolderNormal) holder).mIvDeviceIcon.setImageResource(GetDeviceIcon(device.getDType().getID(),false));
                int color = Build.VERSION.SDK_INT < 23 ? mContext.getResources().getColor(R.color.transparent80) : mContext.getResources().getColor(R.color.transparent80, null);
                ...
                ((ViewHolderNormal) holder).mIvDeviceIcon.setImageResource(GetDeviceIcon(device.getDType().getID(),true));
                int color = Build.VERSION.SDK_INT < 23 ? mContext.getResources().getColor(R.color.white) : mContext.getResources().getColor(R.color.white, null);
                ((ViewHolderNormal) holder).mCardView.setBackgroundColor(color);
    ...
    public int GetDeviceIcon(String deviceType, boolean devicePower){
        switch (deviceType){
            case Comm.DEVICE_TYPE_AC:
                if (devicePower) {
                    return R.drawable.ac_on_64px;
                }
                else return R.drawable.ac_off_64px;
            case Comm.DEVICE_TYPE_EW:
                if (devicePower) {
                    return R.drawable.heater_on;
                }
                else return R.drawable.heater_off;
            case Comm.DEVICE_TYPE_FAN:
                if (devicePower) {
                    return R.drawable.fan_on;
                }
                else return R.drawable.fan_off;
            case Comm.DEVICE_TYPE_SCENE:
                if (devicePower) {
                    return R.drawable.game_30px;
                }
                else return R.drawable.game_30px;
            default:
                if (devicePower) {
                    return R.drawable.light_on_64px;
                }
                else return R.drawable.light_off_64px;
